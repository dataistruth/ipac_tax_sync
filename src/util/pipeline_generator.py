...
def _should_emit_scd_type(table: EffectiveTable, emit_scd_type_1: bool) -> bool:
    if int(table.scd_type) == 2:
        return True
    return emit_scd_type_1 and int(table.scd_type) == 1
...
def _format_object_lines(
    ...
    *,
    ...
    table_config_lines: list[str] = []
    if _should_emit_scd_type(table, emit_scd_type_1):
        table_config_lines.append(f"                scd_type: {_yaml_scd_type(table.scd_type)}")
    if clustering:
        cols = ", ".join(clustering)
        table_config_lines.append(f"                clustering_columns: [{cols}]")
    if table.recon_type != 1:
        table_config_lines.append(f"                recon_type: {table.recon_type}")
    if table_config_lines:
        lines.append("              table_configuration:")
        lines.extend(table_config_lines)
    return lines
...
def generate_client_pipelines_yaml(
    ...
    scd_note = (
        "# pipeline_scd_type_1: omitted (--scd1 off)"
        if not emit_scd_type_1
        else "# pipeline_scd_type_1: enabled (scd_type=1 tables emit SCD_TYPE_1)"
    )
    regen_flags = ""
    if pipeline_split_mode == "recon":
        regen_flags += " --split recon"
    if not emit_scd_type_1:
        regen_flags += " --scd1 off"
    lines = [
        ...
        f"# Regenerate: ipac-delta-sync generate --client {client.client_nm}{regen_flags}",
        "resources:",
        "  pipelines:",
    ]
