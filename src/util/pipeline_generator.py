...
from util.bundle_config import UC_CATALOG_VAR_REF, UC_STAGING_SCHEMA_VAR_REF
...
def _format_object_lines(
    ...
    lines = [
        ...
        f"              destination_schema: {uc_staging_schema_ref}",
...
def _pipeline_resource_lines(
    ...
    lines = [
        ...
        f"      schema: {uc_staging_schema_ref}",
    ...
    for table in tables:
        lines.extend(_format_object_lines(table, client, uc_catalog_ref, uc_staging_schema_ref))
...
def generate_client_pipelines_yaml(
    ...
    uc_staging_schema_ref: str = UC_STAGING_SCHEMA_VAR_REF,
    resolved_uc_staging_schema: str | None = None,
) -> str:
    ...
    staging_schema_comment = resolved_uc_staging_schema or uc_staging_schema_ref
    tier = _tier_for_client(client, cluster_config)
    ...
    lines = [
        ...
        f"# uc_staging_schema: {staging_schema_comment} (from databricks.yml var.uc_staging_schema)",
        f"# uc_schema_resource: {catalog_comment}.{raw_schema} (from client_nm + dest_schema_suffix)",
...
def write_pipeline_yaml(
    ...
    if serial is not None:
        ...
        body = _pipeline_resource_lines(
            client, batch, serial, cluster_config, uc_catalog_ref, uc_staging_schema_ref
        )
        content = "\n".join(header + body) + "\n"
    else:
